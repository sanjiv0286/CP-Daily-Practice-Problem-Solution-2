...
string convert(string t)
{
    ...
    //  return setw(2) << setfill('0') << hour << ":" << setw(2) << setfill('0') << minute << " " << suffix;
    if (h < 10)
        return "0" + to_string(h) + ":" + (min < 10 ? "0" : "") + to_string(min) + " " + suffix;

    return to_string(h) + ":" + (min < 10 ? "0" : "") + to_string(min) + " " + suffix;
}
